...
class CrossOverNode:
    ...
    def children(self):
        ...
        for index, chromosome in enumerate(self.parentChromosomes):
            ...
            if gene is not None:
                if self.itemsToOrder[gene.item][gene.position] is None:
        ...
        for item in range(InputDataInstance.instance.nItems):
            ...
            for position, itemState in enumerate(itemStates):
                ...
                if itemState == 1:
                    ...
                    node.itemsToOrder = [[value for value in row] for row in self.itemsToOrder]
                    # node.itemsToOrder = copy.deepcopy(self.itemsToOrder)
                    node.itemsToOrder[item][position] = 0
        ...
        if self.itemsToOrder[InputDataInstance.instance.nItems][0] > 0:
            ...
            # node.itemsToOrder = copy.deepcopy(self.itemsToOrder)
            node.itemsToOrder[InputDataInstance.instance.nItems][0] -= 1
